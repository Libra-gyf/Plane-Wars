import pygame
import sys
import traceback  # traceback模块用于从程序运行的堆栈中提取，格式化或打印程序目前执行的上下文信息。
...
# Load_Resources.load_music("sound\\game_music.ogg",0.2)
...
def main():
    ...
    # 统计得分
    score = 0
    score_font = pygame.font.Font("font/font.ttf", 36)  # 36 号字体
    ...
    pygame.time.set_timer(SUPPLY_TIME, 30 * 1000)  # 30 * 1000毫秒
    ...
    while running:
        ...
        if life_num and not paused:
            ...
            # 检测我方飞机是否撞毁
            # spritecollide 默认矩形区域是否被撞,添加 mask 方法实现实际区域碰撞检测
            ...
            if me.active:
                ...
                if not (delay % 3):
                    if me_destroy_index == 0:
                        me_down_sound.play()
                    screen.blit(me.destroy_images[e3_destroy_index], me.rect)
                    me_destroy_index = (me_destroy_index + 1) % 4
                    if me_destroy_index == 0:
                        life_num -= 1
                        me.reset()     # 复活后初始化飞机位置
                        pygame.time.set_timer(INVINCIBLE_TIME,3*1000)           # 3s 后调用计时器
        ...
        # 绘制游戏结束画面
            ...
            # 绘制结束画面
            record_score_text = score_font.render("Best:{}".format(record_score), True, WHITE)
